...
PFClusterProducerHbHeCuda::PFClusterProducerHbHeCuda(const edm::ParameterSet& conf)
  ...
  for(const auto& pset : seedFinderConfs){
    if(pset.getParameter< std::string >("detector")==std::string("HCAL_ENDCAP") || pset.getParameter< std::string >("detector")==std::string("HCAL_BARREL1") || pset.getParameter< std::string >("detector")==std::string("ECAL_BARREL") || pset.getParameter< std::string >("detector")==std::string("HCAL_BARREL")){
...
void PFClusterProducerHbHeCuda::produce(edm::Event& e, const edm::EventSetup& es) {
  ...
  for (auto rh: *rechits){
    ...
    for(auto nh: theneighbours)
	{
		...
		std::cout<<"HCAL distance to neighbour "<<k<<": "<<dist<<std::endl;
  ...
  ClusterSeedingCuda::seedingWrapperXYZ_2(d_cuda_pfrh, d_cuda_seedable, rh_size, d_hbthresh, d_hethresh);
