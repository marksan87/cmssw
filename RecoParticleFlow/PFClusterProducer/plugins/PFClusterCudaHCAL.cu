    ...
    if (topoId >-1 && topoId < nRH) {
        ...
        auto computeFromArrays = [&] (float4& pos4, float _frac, int rhInd, bool isDebug) {
            ...
            if(pfrh_layer[rhInd] == PFLayer::HCAL_BARREL1) {
                threshold = 1. / recHitEnergyNormEB_vec[pfrh_depth[rhInd] - 1]; // This number needs to be inverted
            }
            else if (pfrh_layer[rhInd] == PFLayer::HCAL_ENDCAP) { threshold = 1. / recHitEnergyNormEE_vec[pfrh_depth[rhInd] - 1]; }
        ...
        auto compute = [&] (float4& pos4, float& clusterEn, int seedInd, int rhInd) {
            ...
            else if (pfrh_layer[rhInd] == 3) { threshold = 1. / recHitEnergyNormEE_vec[pfrh_depth[rhInd] - 1]; }
                ...
                for (int r = 0; r < nRHTopo; r++) {
                    ...
                    if( pfrh_topoId[i] == pfrh_topoId[j] && pfrh_isSeed[i]==1 ){
                        ...
                        */
                        ...
                        if(pfrh_layer[j] == PFLayer::HCAL_BARREL1) { fraction = clusterEnergy[s] / recHitEnergyNormEB_vec[pfrh_depth[j] - 1] * expf(-0.5 * d2); }
                        else if (pfrh_layer[j] == PFLayer::HCAL_ENDCAP) { fraction = clusterEnergy[s] / recHitEnergyNormEE_vec[pfrh_depth[j] - 1] * expf(-0.5 * d2); }
...
__global__ void hcalFastCluster_original(size_t nRH,
    ...
    if (topoId >-1 && topoId < nRH) {
        ...
        auto computeFromArrays = [&] (float4 pos4, float _frac, int rhInd) {
            float4 rechitPos = make_float4(pfrh_x[rhInd], pfrh_y[rhInd], pfrh_z[rhInd], 1.0);
            float threshold = 0.0;
            if(pfrh_layer[rhInd] == PFLayer::HCAL_BARREL1) {
                threshold = 1. / recHitEnergyNormEB_vec[pfrh_depth[rhInd] - 1]; // This number needs to be inverted
            ...
            for (int s = 0; s < nSeeds; s++) {      // PF clusters
                ...
                for (int r = 0; r < nRHTopo; r++) {
                    ...
                    if (frac > -0.5) {
                        ...
                        if (nSeeds == 1)
                            computeFromArrays(clusterPos[s], frac, j);
                        else {
                            // Check if this is one of the neighboring rechits
                            for(int k=0; k<nNeigh; k++){
                                if(neigh4_Ind[nNeigh*i+k]<0) continue;
...
__global__ void hcalFastCluster_step1( size_t size,
    ...
    if( i<size && j<size){
      ...
      if(pfrh_layer[j] == PFLayer::HCAL_BARREL1) { fraction = pfrh_energy[i] / recHitEnergyNormEB_vec[pfrh_depth[j] - 1] * expf(-0.5 * d2); }
      else if (pfrh_layer[j] == PFLayer::HCAL_ENDCAP) { fraction = pfrh_energy[i] / recHitEnergyNormEE_vec[pfrh_depth[j] - 1] * expf(-0.5 * d2); }
...
void PFRechitToPFCluster_HCAL_CCLClustering(int nRH,
    ...
    //hcalFastCluster<<<32, 128>>>( nRH, pfrh_x,  pfrh_y,  pfrh_z,  pfrh_energy, pfrh_topoId,  pfrh_isSeed,  pfrh_layer, pfrh_depth, neigh4_Ind, pcrhfrac, pcrhfracind, fracSum, rhCount);
    
    hcalFastCluster_serialize<<<1,1>>>( nRH, pfrh_x,  pfrh_y,  pfrh_z,  pfrh_energy, pfrh_topoId,  pfrh_isSeed,  pfrh_layer, pfrh_depth, neigh4_Ind, pcrhfrac, pcrhfracind, fracSum, rhCount);
